...
class DewormingReportController extends Controller
{
    public function select(Request $request)
    {
        if (Auth::user()->permission == 1) {
            ...
            return view('admin.report.deworming.select', compact('farms', 'communityCats'));
        } else {
    ...
    public function report(Request $request)
    {
        ...
        if (Auth::user()->permission == 1) {
            if ($fOrC == 'f') {
                $dewormings = Deworming::whereFarm_id($farmOrComId)->whereNotIn('animal_info_id', isCulling())->get();
            } else {
                $dewormings = Deworming::whereCommunity_cat_id($farmOrComId)->whereNotIn('animal_info_id', isCullingUser())->get();
            }
        }
        return view('admin.deworming.index', compact('dewormings'));
    }
    ...
    public function excel($farmId, $communityCatId)
    {
        ...
        return Excel::download(new DewormingExport($farm, $farm_id), 'Deworming.xlsx');
    }
    ...
    public function pdf($farmId, $communityCatId)
    {
        ...
        if (Auth::user()->permission == 1) {
            $dewormings = Deworming::where($farm, '=', $farm_id)->whereNotIn('animal_info_id', isCulling())->get();
        } else {
            $dewormings = Deworming::where('user_id', Auth::user()->id)->whereNotIn('animal_info_id', isCullingUser())->get();
        }
